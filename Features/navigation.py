...
def run(screen, predictions):
    fin = screen
    #fin = filterWhite(screen)
    vert = np.array([[100,550],[375,350],[450,350],[800,550]], np.int32)
    
    img_path = SaveImageFromArray(screen,"logs/navigation_images")
    fin = edgeDetect(fin)
    fin = roi(fin, [vert])
...
##        cv2.imshow("edgedetection", fin)
    ...
    try:
        ...
        append_data("logs/navigation_data.csv", "entered","entered") 
        if (results is not None):
            if (results[0] > 400):
                direction = "Turn Left"
                if predictions[-1] != direction:
                    predictions.append(direction)
                    append_data("logs/navigation_data.csv", img_path,direction)
                    return ("Turn Left")
            ...
            if predictions[-1] != direction:
                    ...
                    append_data("logs/navigation_data.csv", img_path,"Go Straight")
                    return("Go straight")
...
def get_direction():
    try:
       data = pd.read_csv("logs/navigation_data.csv")
       return (data.iloc[-1]['response'])
    except Exception as e:
        print("Exception", e)
    #
...
def navigation():
    ...
    try:
        ...
        run(screen,predictions)
        # cv2.imshow("Navigation", run(screen,predictions))
        cap.release()
